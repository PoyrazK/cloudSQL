 ...
 * @brief Column-oriented storage implementation
 */
...
bool ColumnarTable::create() {
    ...
    for (size_t i = 0; i < schema_.column_count(); ++i) {
        ...
        std::ofstream n_out(base + ".nulls.bin", std::ios::binary);
        std::ofstream d_out(base + ".data.bin", std::ios::binary);
...
bool ColumnarTable::append_batch(const executor::VectorBatch& batch) {
    for (size_t i = 0; i < schema_.column_count(); ++i) {
        std::string base = name_ + ".col" + std::to_string(i);
        std::ofstream n_out(base + ".nulls.bin", std::ios::binary | std::ios::app);
        std::ofstream d_out(base + ".data.bin", std::ios::binary | std::ios::app);
        if (!n_out.is_open() || !d_out.is_open()) return false;
...
bool ColumnarTable::read_batch(uint64_t start_row, uint32_t batch_size, executor::VectorBatch& out_batch) {
    ...
    for (size_t i = 0; i < schema_.column_count(); ++i) {
        std::string base = name_ + ".col" + std::to_string(i);
        std::ifstream n_in(base + ".nulls.bin", std::ios::binary);
        std::ifstream d_in(base + ".data.bin", std::ios::binary);
